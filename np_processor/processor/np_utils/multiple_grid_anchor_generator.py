...
import numpy as np
from ..np_utils import grid_anchor_generator, anchor_generator
...
class MultipleGridAnchorGenerator(anchor_generator.AnchorGenerator):
    ...
    def _generate(self, feature_map_shape_list, im_height=1, im_width=1):
        ...
        anchor_grid_list = []
        min_im_shape = np.minimum(im_height, im_width)
        ...
        for feature_map_index, (grid_size, scales, aspect_ratios, stride,
            ...
            anchor_indices = feature_map_index * np.ones([num_anchors_in_layer])
            tiled_anchors.add_field('feature_map_index', anchor_indices)
            anchor_grid_list.append(tiled_anchors)
        print("anchor_grid_list:", anchor_grid_list)
