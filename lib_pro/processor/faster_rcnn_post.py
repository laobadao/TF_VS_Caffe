...
class ProposalTF(KerasCustomLayer):

    def __init__(self):
        super(KerasCustomLayer, self).__init__()
        self._scales = (0.25, 0.5, 1.0, 2.0)
        self._aspect_ratios = (0.5, 1.0, 2.0)
        self._anchor_stride = (16, 16)
        self._max_proposals = 100
        self._nms_score_threshold = 0.0
        self._nms_iou_threshold = 0.699999988079
    ...
    def get_config(self):
        config = {
            ...
            'anchor_stride': self._anchor_stride,
            'max_proposals': self._max_proposals,
            'nms_score_threshold': self._nms_score_threshold,
            'nms_iou_threshold': self._nms_iou_threshold
        }

        config_data = super(ProposalTF, self).get_config()
        config_data.setdefault('config', config)
        return config_data

    def set_config(self, scales=(0.25, 0.5, 1.0, 2.0), aspect_ratios=(0.5, 1.0, 2.0), anchor_stride=(16, 16),
                   max_proposals=100, nms_score_threshold=0.0, nms_iou_threshold=0.699999988079):
    ...
    def proposal(self, preprocessed_inputs, box_encodings, class_predictions_with_background,
        ...
        box_predictions = self._first_stage_box_predictor_predict([rpn_box_predictor_features], [box_encodings],
        ...
        first_stage_max_proposals = self._max_proposals
    ...
    def _postprocess_rpn(self,
        ...
        Returns:
          ...
          num_proposals: A Tensor of type `int32`. A 1-D tensor of shape [batch]
            ...
            the batch.
        """
        first_stage_nms_score_threshold = self._nms_score_threshold
        first_stage_nms_iou_threshold = self._nms_iou_threshold
    ...
    def _first_stage_box_predictor_predict(self, image_features, box_encodings, class_predictions_with_backgrounds,
        ...
        for (image_feature, box_encoding, class_predictions_with_background, num_predictions_per_location) in zip(
            ...
            shapes = tf.stack([combined_feature_map_shape[0],
                               combined_feature_map_shape[1] * combined_feature_map_shape[
                                   2] * num_predictions_per_location,
                               1,
                               _box_code_size])
    ...
    def second_stage_box_predictor(self, preprocessed_inputs, box_encoding_reshape, class_prediction_reshape,
                                   rpn_features_to_crop,
                                   rpn_box_encodings,
        ...
        if len(num_anchors_per_location) != 1:
            raise RuntimeError('anchor_generator is expected to generate anchors '
                               'corresponding to a single feature map.')
        box_predictions = self._first_stage_box_predictor_predict([rpn_box_predictor_features], [rpn_box_encodings],
                                                                  [rpn_objectness_predictions_with_background],
                                                                  num_anchors_per_location)
        ...
        result_output = self.second_postprocess(prediction_dict, true_image_shapes)

        return result_output


    def second_postprocess(self, prediction_dict, true_image_shapes):
        ...
        """
        postprocessed_tensors = self._postprocess_box_classifier(
        ...
        return self._add_output_tensor_nodes(postprocessed_tensors)

    def _add_output_tensor_nodes(self, postprocessed_tensors,
        ...
        Returns:
          A dictionary containing:
            ...
            `detection_masks`:
              ...
              masks.
        """
        _first_stage_max_proposals = self._max_proposals
...
class RoiPoolingTF(KerasCustomLayer):

    def __init__(self):
        super(KerasCustomLayer, self).__init__()
        self._initial_crop_size = 14
        self._maxpool_kernel_size = 2
        self._maxpool_stride = 2
    ...
    def get_config(self):
        config = {
            'initial_crop_size': self._initial_crop_size,
            'maxpool_kernel_size': self._maxpool_kernel_size,
            'maxpool_stride': self._maxpool_stride,
        }

        config_data = super(RoiPoolingTF, self).get_config()
        config_data.setdefault('config', config)
        return config_data

    def set_config(self, initial_crop_size=14, maxpool_kernel_size=2, maxpool_stride=2):
    ...
    def roi_pooling(self, features_to_crop,
        ...
        ksize = self._maxpool_kernel_size
        strides = self._maxpool_stride

        print("cropped_regions shape:", cropped_regions.shape)
        ...
        max_pooled = slim.max_pool2d(
            ...
            [ksize, ksize],
            stride=strides)
        max_pooled = tf.expand_dims(max_pooled, axis=0)
