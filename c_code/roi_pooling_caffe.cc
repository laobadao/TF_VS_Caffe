...
class RoiPoolingCaffeOp : public OpKernel {
 public:
  explicit RoiPoolingCaffeOp(OpKernelConstruction* context) : OpKernel(context) {
    // Get the pool height
...
REGISTER_KERNEL_BUILDER(Name("RoiPoolingCaffe").Device(DEVICE_CPU).TypeConstraint<float>("T"), RoiPoolingCaffeOp<CPUDevice, float>);